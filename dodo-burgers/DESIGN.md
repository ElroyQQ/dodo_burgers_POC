...
**2026-09-24: converted from vertical to horizontal-scroll.** The page is a single fixed-height (`100vh`) flex row (`.track`) of full-viewport (`100vw`) panels; the site advances sideways, not down. A fixed header overlays every panel; a thin progress bar at the bottom edge shows position across the whole track. Panel order (revised 2026-09-24): hero → footer/visit info → "why us" → field-log/story → menu → FAQ → builder (pick ingredients) → build summary (last, deliberately — it lands right as the chase bar's chef catches the dodo, so "burger complete" and "chase complete" pay off together). The builder was deliberately split into two panels (pick, then review/add) rather than kept as one tall vertical stack, since a horizontal panel has to fit its content in one viewport height. Mouse wheel input is redirected from vertical to horizontal scroll (`deltaY` drives `scrollLeft`); trackpad/touch horizontal swipes pass through natively; arrow/Page/Home/End keys move panel-to-panel; nav links call `scrollIntoView({inline:'start'})` instead of relying on native anchor jump. Every panel's content sits in a `.panel-inner` with `overflow-y:auto` as a safety net (normally invisible/inactive) for any viewport short enough that a panel's content doesn't fit — the wheel handler checks this before hijacking, so a genuinely tall panel still scrolls vertically in place before advancing horizontally.
