...
- **[dodo-burgers-vercel-supabase/](dodo-burgers-vercel-supabase/CLAUDE.md)** — a fork of `dodo-burgers/` swapping Cloudflare Pages + D1 for Vercel + Supabase Auth, to test the same login/signup/logout feature on a different stack. Plain HTML/CSS/JS plus one CDN-loaded dependency (`supabase-js`), no build step. **Its own separate git repository** — excluded from this directory's `.gitignore`, never `git add`ed from here.

## Working across projects

- `cd` into the relevant subfolder before doing anything project-specific — commands run from here (like `git status`, `git add`) apply to *this* directory's repo, which only tracks `dodo-burgers/`. `dividend-capture-analysis/`, `nuzzle-pet-care/`, and `dodo-burgers-vercel-supabase/` are separate repos; always run their git commands from inside their own folders.
